    ...
    return (
        <div className={styles.activity_menu}>
            <label className={styles.custom_field} htmlFor="activityName">
                    <input type="text" id="activityName" name="activityName" required placeholder=" "/>
                    <span className={styles.placeholder}> Nombre de la Actividad</span>
                    <span className={styles.border} />
                </label>

            <div className={styles.grid_item_2}><b>Temperatura Ideal</b></div>
            <label className={styles.custom_field_2} htmlFor="idealTemp">
                <input type="number" id="idealTemp" name="idealTemp" required placeholder=" " min="-50" max="50"/>
                <span className={styles.input_suffix}>ºC</span>
                <span className={styles.border} />
            </label>

            <div className={styles.grid_item_2}><b>Humedad (Opcional)</b></div>
            <label className={styles.custom_field_2} htmlFor="idealHumidity">
                <div className={styles.slider}>
                    <input type="range" id="idealHumidity" min="0" max="100" value={humidityValue} onChange={handleHumidityChange} />
                    <progress min="0" max="100" value={humidityValue}/>
                </div>
                <div className={styles.sliderValue}>{humidityValue}%</div>
            </label>

            <div className={styles.grid_item_2}><b>Preferencia de Rayos UV (Opcional)</b></div>
            <label className={styles.custom_field_2} htmlFor="uvIndexSlider">
            <div className={styles.slider}>
                <input type="range" id="uvIndexSlider" min="0" max={uvScaleLabels.length - 1} step="1" value={uvIndex} onChange={handleUvIndexChange} />
                <progress min="0" max={uvScaleLabels.length - 1} value={uvIndex} />
            </div>
            <div className={styles.sliderValue}>{uvScaleLabels[uvIndex]}</div>
        </label>

            <div className={styles.grid_item_2}><b>Velocidad del Viento (Opcional)</b></div>
            <label className={styles.custom_field_2} htmlFor="windSpeedSlider">
            <div className={styles.slider}>
                <input type="range" id="windSpeedSlider" min="0" max="50" step="5" value={windSpeed} onChange={handleWindSpeedChange} />
